FROM alpine:22.04 as builder

# Atualizando pacotes
RUN apk update --no-cache 

# Instalar Terraform
RUN apk-get install wget unzip curl --no-cache
...
RUN mv terraform /usr/local/bin/
 ...
 # Instalar o Python
RUN apk-get install python3.9 --no-cache 

RUN apk install python3-pip --no-cache
...
RUN apk-get update && apk-get install --no-cache python3.9 python3-pip
RUN python3 -m pip install -r requirements.txt
